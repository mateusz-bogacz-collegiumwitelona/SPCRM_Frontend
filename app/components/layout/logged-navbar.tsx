...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex h-20 items-center justify-between border-b border-black/5 bg-brand px-4 shadow-[0_4px_12px_rgba(0,0,0,0.2)]">
      <div className="relative ml-auto mr-2 md:mr-6">
        <button
          type="button"
        ...
        {isOpen && (
          <>
            <button className="fixed inset-0 z-40" onClick={() => setIsOpen(false)} />

            <div className="absolute right-0 top-full mt-2 w-56 rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5 z-50">
              <Link
                to="/help"
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-3 px-4 py-3 text-sm text-gray-700 transition-colors hover:bg-blue-50 hover:text-brand"
              >
                <LifeBuoy size={18} />
