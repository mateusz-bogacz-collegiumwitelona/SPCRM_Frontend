...
import type { AddCurrencyRequestPayload } from '~/interfaces/currency';
...
export function AddCurrencyDialog({ isOpen, onClose, onSave, isLoading }: AddCurrencyDialogProps) {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    if (code.trim().length !== 3)
      validationErrors.push('Kod waluty musi składać się dokładnie z 3 znaków (np. PLN, EUR).');
    ...
    try {
      ...
      const fallback = responseData?.message || apiError.message || 'Nie udało się dodać waluty.';
