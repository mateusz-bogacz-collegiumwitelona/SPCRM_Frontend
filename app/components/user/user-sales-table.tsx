...
const columns = [
  ...
  columnHelper.accessor('status', {
    ...
    cell: (info) => {
      ...
      return (
        <span
          className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium whitespace-nowrap ${status.bgColor} ${status.textColor}`}
        >
          {status.label}
        </span>
      );
...
export const UserSalesTable = ({ userId }: { readonly userId: string }) => {
  ...
  const totalItems = data?.totalItems || data?.totalCount || sales.length;
    ...
    isError && !isErrorDismissed
      ? {
          title: getErrorMessage(
            ...
            responseData?.message || activeError?.message || 'Błąd pobierania szans sprzedaży.',
          ),
        }
      : null;

  const isFilterActive = Boolean(statusFilter || dateFrom || dateTo);
    ...
    <div className="bg-white border border-gray-200 rounded-xl shadow-xs overflow-hidden">
      <div className="px-4 py-3 border-b border-gray-200 bg-gray-50/70 flex flex-col lg:flex-row lg:items-center justify-between gap-2.5">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <h2 className="text-sm font-semibold text-gray-900">Szanse sprzedaży</h2>
            ...
            {showFilters && (
              <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-gray-200 rounded-lg shadow-lg z-50 p-3 text-xs space-y-3">
                <div className="flex justify-between items-center border-b border-gray-100 pb-1.5">
                  <span className="font-semibold text-gray-800">Filtruj sprzedaż</span>
                  <button
                    type="button"
